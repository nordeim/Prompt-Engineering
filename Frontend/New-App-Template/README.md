**Project Goal:** Combining React 19, Tailwind CSS 4.0, and Shadcn UI (with Vite) will give you a cutting-edge and highly efficient development environment for your "Avant-Garde" UI with "Intentional Minimalism."

---

### 🛠️ Easiest Scaffolding Commands (React 19 + Tailwind CSS 4.0 + Shadcn UI with Vite)

This process leverages Vite for its speed and flexibility, then integrates the other components.

#### **Step 1: Create a New Vite Project with React (targeting React 19)**

You can initialize a new Vite project using the `npm create vite@latest` command. While `create-vite` might not explicitly offer "React 19" yet, you can typically use the latest stable React template and then upgrade the React dependencies.

Okay, let's get you set up with your ideal stack!

Based on my research, while React 19 is now stable (as of December 2024) and Tailwind CSS 4.0 is available in early access/pre-release, there isn't a single, magic command that scaffolds all three of these bleeding-edge tools together perfectly. However, I can provide you with the most streamlined sequence of commands to achieve this with a **Vite + React** base.

The process involves a few steps to ensure each component is correctly integrated.

---

### 🚀 Step-by-Step Setup: React 19 + Tailwind CSS 4.0 + Shadcn UI with Vite

Here's the easiest command sequence:

#### **1. Create Your Vite + React Project (with React 19)**

First, we'll create a new Vite project. While Vite's default React template might still provision React 18, we'll explicitly upgrade to React 19 immediately after.

```bash
# 1. Create a new Vite project
npm create vite@latest my-avant-garde-app -- --template react-ts

# 2. Navigate into your new project directory
cd my-avant-garde-app

# 3. Install initial dependencies
npm install

# 4. Explicitly upgrade React and React-DOM to their latest stable (React 19)
npm install react@latest react-dom@latest

# 5. Update TypeScript types for React (if you chose TypeScript)
npm install --save-dev @types/react@latest @types/react-dom@latest

# You can optionally start the dev server to confirm React 19 is working:
# npm run dev
# Check your browser console or a component to see React.version
```
*Self-correction*: React 19.0.0 was released in December 2024, and Vite templates are catching up. It's best to explicitly install `@latest` to ensure you get React 19.

#### **2. Install and Configure Tailwind CSS 4.0**

This step will integrate the alpha version of Tailwind CSS 4.0. Remember that since it's a pre-release, the setup might have minor variations in the future when it reaches stable.

```bash
# 1. Install Tailwind CSS 4.0 and its Vite plugin
npm install tailwindcss@next @tailwindcss/vite
# @next installs the latest pre-release (4.0)

# Optional: Install official Tailwind plugins if you plan to use them (e.g., for forms or typography)
# npm install @tailwindcss/forms @tailwindcss/typography
```

Next, you need to configure Vite and your main CSS file.

**a. Update `vite.config.ts` (or `vite.config.js`)**
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      '@': '/src', // This is a common alias, useful for Shadcn UI later
    },
  },
});
```

**b. Update Your Main CSS File**

Tailwind CSS 4.0 simplifies the CSS import. You no longer need `@tailwind base; @tailwind components; @tailwind utilities;`. Instead, you use a single `@import`.

Open your main CSS file (e.g., `src/index.css` or `src/App.css`) and replace its content with:

```css
/* src/index.css or src/App.css */
@import "tailwindcss";

/* Optional: If you installed official plugins, you can import them here */
/* @plugin '@tailwindcss/forms'; */
/* @plugin '@tailwindcss/typography'; */

/* Add your own global styles below this */
```

#### **3. Integrate Shadcn UI Components**

Shadcn UI is built to be framework-agnostic but designed primarily for React and relies on Tailwind CSS. The process involves initializing Shadcn UI in your project and then adding components.

Since Tailwind CSS 4.0 might change how `tailwind.config.js` is typically handled (moving towards CSS-first configuration), the standard Shadcn UI initialization might need slight adjustments or workarounds in the future. As of now, Shadcn UI still expects a `tailwind.config.js` for its configuration and theme customization.
...
This command will prompt you to configure `components.json` and will try to set up your `tailwind.config.js` file.

*   **For `tailwind.config.js`:** When prompted, choose the option that creates or modifies `tailwind.config.js`. Shadcn UI will add paths for its components and extend Tailwind's theme.
*   **Base style**: Choose `"default"` or `"new-york"`.
*   **Color**: You can pick one or leave it default.
*   **CSS variables**: Enable them for easier theming.
*   **Global CSS file**: Specify the path to your main CSS file (e.g., `src/index.css` or `src/App.css`).

**b. Add Your First Shadcn UI Component**

After initialization, you can start adding components.
...
This command will:
*   Add the `Button` component files to your specified components directory (e.g., `src/components/ui/button.tsx`).
*   Update your `tailwind.config.js` to ensure the component's styles are included.
*   Potentially add necessary dependencies (e.g., `class-variance-authority`, `clsx`, `lucide-react`).

---

### 🎉 Summary of Commands

Here's the combined sequence:

```bash
# 1. Create Vite React project & upgrade to React 19
npm create vite@latest my-avant-garde-app -- --template react-ts
cd my-avant-garde-app
npm install
npm install react@latest react-dom@latest
...
# 2. Install and configure Tailwind CSS 4.0
npm install tailwindcss@next @tailwindcss/vite
# (Optional) npm install @tailwindcss/forms @tailwindcss/typography

# --- MANUALLY UPDATE FILES ---
# A. Update vite.config.ts (add tailwindcss() to plugins)
# B. Update src/index.css (replace content with @import "tailwindcss";)
# --- END MANUAL UPDATE ---

# 3. Initialize Shadcn UI
npx shadcn-ui@latest init
# Follow the prompts:
# - Choose your base style and color.
# - Confirm your global CSS file path (e.g., src/index.css).
# - Allow it to create/update tailwind.config.js.

# 4. Add your first Shadcn UI component (example: Button)
npx shadcn-ui@latest add button
