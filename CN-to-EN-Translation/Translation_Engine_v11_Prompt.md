...
def translate(
    ...
    # Token budget & Draft-Lock tier (size-based segmentation trigger)
    word_count = len(payload.split())
...
def main():
    ...
    # Draft-Lock segmentation signal
    if should_segment(payload):
        print(f"[INFO] Draft-Lock tier: Segmented ({len(payload.split())} words)", file=sys.stderr)
